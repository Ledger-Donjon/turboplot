...
impl Camera {
    ...
    pub fn world_to_screen_x(&self, viewport: &Rect, ppp: f32, x: Fixed) -> f32 {
        (Fixed::from_num(viewport.width() * ppp / 2.0) + (x - self.shift.x) / self.scale.x)
            .to_num::<f32>()
            / ppp
    }

    pub fn screen_to_world_x(&self, viewport: &Rect, ppp: f32, x: f32) -> Fixed {
